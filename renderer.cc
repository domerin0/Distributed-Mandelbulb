		   ...
		   unsigned char* image)
{
  ...
  for(j = 0; j < height; j++)
  {
    ...
    #pragma omp parallel for private(i, k, to, color, farPoint, pix_data) shared(image) schedule(dynamic)
    for(i = 0; i <width; i++)
    {
      ...
      to.x = farPoint[0] - camera_params.camPos[0];
      to.y = farPoint[1] - camera_params.camPos[1];
      to.z = farPoint[2] - camera_params.camPos[2];
